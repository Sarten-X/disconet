...
CursesOutput::CursesOutput() :Output()
{
  //ctor
}
...
void CursesOutput::create_box(int y, int x, int w, int h)
  {
    mvaddch(y, x, ' ');
    mvaddch(y, x + w, ' ');
    mvaddch(y + h, x, ' ');
  ...
  void CursesOutput::fill_rect(int y, int x, int w, int h)
  {
    mvhline(y, x, ' ', w);
    for (int n = 1; n < h; n++)
      mvhline(y+n, x, ' ', w);
  ...
  void CursesOutput::initialize_drawing()
  {
  ...
  void CursesOutput::draw_block(int w, int h, int type)
  {
    int colors = 8;
  ...
  void CursesOutput::uninitialize_drawing() {
    getch();

    endwin();
  }

  void CursesOutput::paint_drawing(const std::vector<net_state> objects, const std::map<dataType_t, net_state> aggregate, double xmultipler, double ymultipler) {
  ...
  void CursesOutput::refresh_drawing() {
    refresh();
    int chr = getch();
    switch(chr) {
      case 'q':
